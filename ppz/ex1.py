ztype=["z","m","b"]
def prepositions(txt):
    prepositions=0
    for word in txt:
        if ( word.startswith(ztype[0]) or
            word.startswith(ztype[1]) or
            word.startswith(ztype[2])
        ) and not(
            word.endswith(ztype[0]) or
            word.endswith(ztype[1]) or
            word.endswith(ztype[2])
        ):
            prepositions+=1
    return prepositions
...
def verbs(txt):
    ...
    for word in verb_list:
        if not(
            word.startswith(ztype[0]) or
            word.startswith(ztype[1]) or
            word.startswith(ztype[2])
        ):
            first_person_verbs.append(word)
    return len(verb_list),len(first_person_verbs)

def set_criteria():
    t="zmbtshjpnwlrcxkqvdgf"
    sort_criteria={}
    for i in range(len(t)):
        sort_criteria[t[i]]=i
    return sort_criteria

def bigger(item1,item2):
    ## if str(item1) > str(item2): return true
    if item1==item2:
        return False
    if len(item1)>len(item2) and item1.startswith(item2):
        return True
    for i in range(min(len(item1),len(item2))):
        if criteria[item1[i]]==criteria[item2[i]]: continue
        if criteria[item1[i]]>criteria[item2[i]]:
            return True
        else: return False
    return False
...
def custom_sort(txt):
    for i in range(1,len(txt)):
        key=txt[i]
        j=i-1
        while j>=0 and bigger(txt[j],key):
            txt[j+1]=txt[j]
            j-=1
        txt[j+1]=key
    return txt
